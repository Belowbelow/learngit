    ...
    for (i, x, y, z, l) in zip(imgLinks, carNames, carPrices, carScores, carLinks):
        ...
        try:
            field = x + '\t' + y.replace("万", '').split('-')[0] + '\t' + y.replace("万", '').split('-')[1] + \
                '\t' + z.replace("分", '') + '\t'
            finalGet = getKouBeiInfo(l, field, i)
            dataTrunk.append(finalGet + '\n')
...
def parserPersonKB(url):
    ...
    try:
        ...
        if(len(orgInfoPart1) == 4 and len(orgInfoPart2) == 2 and len(orgInfoPart3) == 10):
            ...
            pass
    except UnboundLocalError and IndexError and AttributeError as e:
        print(e)
...
#main()
getMainInfo("http://koubei.bitauto.com/tree/xuanche/?page=1&s=6&order=1")